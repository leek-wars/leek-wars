...
CodeMirror.defineMode('leekscript-v2', (config, modeOptions) => {
	var indentUnit = config.indentUnit;
	var statementIndent = modeOptions.statementIndent;
	var jsonMode = modeOptions.json;
	...
	var keywords = function() {
		...
		var jsKeywords = {
			...
			"for": kw('for'), "in": kw('operator'), "class": kw('class'), 'let': kw('let'), 'const': kw('const'), 'then': kw('then'), 'end': kw('end'), 'new': kw('new'),
		...
		for (var c of ['Null', 'Boolean', 'Number', 'String', 'Array', 'Interval', 'Set', 'Map', 'Object', 'Function', 'Class', 'System']) {
			jsKeywords[c] = {type: "variable", style: "lsclass"}
		}
		for (var c of ['Fight', 'Field', 'Color', 'Entity']) {
			jsKeywords[c] = {type: "variable", style: "lsclass"}
		}
		return jsKeywords
	}()
	...
	function funarg(type, value) {
		if (type == "variable") {register(value); return cont(maybetype);}
	}
	...
	return {
		startState: function(basecolumn) {
			return {
				...
				localVars: modeOptions.localVars,
				globalVars: modeOptions.globalVars,
				context: modeOptions.localVars && {vars: modeOptions.localVars},
				indented: 0
			};
		},
		...
		indent: function(state, textAfter) {
			...
			else if (lexical.info == "switch" && !closing && modeOptions.doubleIndentSwitch != false)
