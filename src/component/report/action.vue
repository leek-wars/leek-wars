	...
	<div>
		...
		<div v-else-if="type === ActionType.USE_CHIP">
			<i18n path="fight.leek_cast">
				<leek :leek="leek" place="leek" />
				<b place="chip">{{ $t('chip.' + LeekWars.chips[LeekWars.chipTemplates[action.params[3]].item].name) }}</b>
			</i18n>
			<span v-if="action.params[4] === 2">... {{ $t('effect.critical') }}</span>
		</div>
		<i18n v-else-if="type === ActionType.SET_WEAPON" path="fight.leek_take_weapon" tag="div">
			<leek :leek="leek" place="leek" />
			<b place="weapon">{{ $t('weapon.' + LeekWars.weapons[LeekWars.weaponTemplates[action.params[2]].item].name) }}</b>
		</i18n>
		...
		<i18n v-else-if="type === ActionType.TP_LOST" tag="div" path="fight.leek_loose_x">
			<leek :leek="leek" place="leek" />
			<b place="value" class="color-tp">{{ $t('fight.n_tp', [action.params[2]]) }}</b>
		</i18n>
		<i18n v-else-if="type === ActionType.LIFE_LOST" tag="div" path="fight.leek_loose_x">
			<leek :leek="leek" place="leek" />
			<b place="value" class="color-life">{{ $t('fight.n_life', [action.params[2]]) }}</b>
		</i18n>
		<i18n v-else-if="type === ActionType.MP_LOST" tag="div" path="fight.leek_loose_x">
			<leek :leek="leek" place="leek" />
			<b place="value" class="color-mp">{{ $t('fight.n_mp', [action.params[2]]) }}</b>
		</i18n>
		...
		<i18n v-else-if="type === ActionType.BOOST_VITA" tag="div" path="fight.leek_win_x">
			<leek :leek="leek" place="leek" />
			<b place="value" class="color-life">{{ $t('fight.n_vita', [action.params[2]]) }}</b>
		</i18n>
