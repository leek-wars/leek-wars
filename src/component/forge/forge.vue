<template>
	<div class="forge">
		<div class="grid">
			<div v-for="(item, i) in forge" :key="i" class="cell" :class="{['cell' + i]: true, active: !!item, building: item && building, partial: slotStates[i] === 'partial', missing: slotStates[i] === 'missing', removable: !!item && !!component, fusing: fusing && !!item}" :style="{ '--dx': CELL_VECTORS[i][0] + 'px', '--dy': CELL_VECTORS[i][1] + 'px' }" @click="component && removeAlteration(i)">
