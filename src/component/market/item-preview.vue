<template lang="html">
	<div v-if="item" class="item-preview" :class="category">
		...
		<component-preview v-else-if="item.type === ItemType.COMPONENT" :component="(LeekWars.components[item.params] as any)" :alterations="instance?.stats" :family="LeekWars.alterations?.component_families?.[item.params]" @update:model-value="$emit('update:modelValue', $event)" />
