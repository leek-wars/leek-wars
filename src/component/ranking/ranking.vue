				...
				<div class="scroll-x">
					...
					<table v-else-if="displayCategory.startsWith('boss')" class="ranking large boss-ranking">
						...
						<tr v-for="row in (ranking as unknown as BossRow[])" :key="row.id" :class="{me: row.me, highlight: searchResult == row.rank}">
							...
							<td>{{ row.turns }}</td>
							<td>{{ row.leeks }}</td>
							<td>{{ Math.pow(row.power / row.leeks, 1 / LeekWars.POWER_FACTOR).toFixed(2) }}</td>
...
<script setup lang="ts">
	...
	interface FunRanking {
		title: string
		value: string
		value_type: 'number' | 'money' | 'distance'
		ranking: {
			ranking: FunRankingRow[]
			farmer_rank: number
			farmer_value: number
		}
	}
	...
	function goToResult(event: Record<string, unknown>) {
		...
		LeekWars.goToRanking(event.type as string, 'talent', event.id as number)
	}
