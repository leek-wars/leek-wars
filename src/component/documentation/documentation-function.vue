	...
	<div v-if="fun">
		<h2 class="searchable">
			...
			<span v-if="fun.return_type != 0">
				: <span v-if="fun.return_type != -1" class="argument"> {{ $t('documentation.arg_type_' + fun.return_type) }}</span><span v-else>?</span>&nbsp;{{ fun.return_name }}
			</span>
		</h2>
		<div v-if="fun.deprecated" class="deprecated-message">Cette fonction est dépréciée.</div>
		<div class="content searchable" v-html="$t('documentation.func_' + fun.real_name)"></div>
		...
		<div v-if="fun.return_type != 0">
			...
			<ul>
				<li>{{ fun.return_name }} : <span class="searchable" v-html="$t('documentation.func_' + fun.real_name + '_return')"></span>
				</li>
			</ul>
		</div>
		<div class="min-stats">
